<script>
	...
	$effect(() => {
		// Ensure this runs after the component is mounted in the browser
...
<nav class="flex justify-between px-6 py-5 bg-[#374993] text-white relative z-10">
	<!-- Logo Reveal Wrapper -->
	<div class="overflow-hidden h-10 w-10">
		<img
			src="/logo.png"
			alt="Surcos Logo"
	...
	<ul
		...
		class="flex-col md:flex md:flex-row gap-6 items-start md:items-center text-lg font-['Space_Grotesk'] absolute md:static top-full right-6 bg-[#374993] md:bg-transparent p-4 md:p-0 rounded-md md:rounded-none shadow-md md:shadow-none z-10 w-64 md:w-auto {menuOpen ? 'flex' : 'hidden'}"
	>
		...
		<li class="block md:hidden">
			<button
				...
				class="hover:underline focus:outline-none w-full text-left"
			>
